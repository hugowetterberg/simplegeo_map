...
ClusterMarker.prototype._clusterMarker=function ($clusterGroupIndexes){
  var $clusterGroupBounds=new GLatLngBounds(), i, $clusterMarker, $clusteredMarkers=[], $marker, $this=this, $mapMarkers=this._mapMarkers, clusterCount = 0, markerId = [];
  for(i=$clusterGroupIndexes.length-1; i>=0; i--){
    $marker=$mapMarkers[$clusterGroupIndexes[i]];
    $marker.index=$clusterGroupIndexes[i];
    $clusterGroupBounds.extend($marker.clusterBounds.getNorthEast());
    $clusterGroupBounds.extend($marker.clusterBounds.getSouthWest());
    $clusteredMarkers.push($marker);
    clusterCount = clusterCount + $marker.clusterCount;
    markerId = markerId.concat($marker.markerId);
  }
  markerId = markerId.slice(0, Drupal.settings.simpleGeoMap.maxNodes);

  // This uses a public function defined by simplegeo_tileservice_map.js.
  $clusterMarker= SimpleGeoMap.newMarker('cluster', $clusterGroupBounds.getCenter(), clusterCount, $clusterGroupBounds, markerId, Drupal.t("Show items"), this._map);
